...
export default class UserProfile {
  constructor(
    ...
    if (house && (house.ownershipStatus === undefined)) throw new ValidationError('House ownership status is required');
    if (vehicle && vehicle.year === undefined) throw new ValidationError('Vehicle year is required');
    ...
    if (vehicle && vehicle.year < 0) throw new ValidationError('Vehicle year must be greater or equal 0');
    ...
    if (house && (house.ownershipStatus !== 'owned' && house.ownershipStatus !== 'mortgaged')) throw new ValidationError('House ownership status must be either owned or mortgaged');
  }
}
